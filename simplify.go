...
func heapUp(heap *minHeap, i int) {
	var object = heap.array[i]
	for i > 0 {
		var up = ((i + 1) >> 1) - 1
		var parent = heap.array[up]
		if compare(object, parent) >= 0 {
			break
		}
		parent.index = i
		heap.array[parent.index] = parent
		i = up
		object.index = i
		heap.array[object.index] = object
	}
}
func (heap *minHeap) push(args ...*triangle) int {
	for _, object := range args {
		object.index = len(heap.array)
		heap.array = append(heap.array, object)
		heapUp(heap, object.index)
	}
	return len(heap.array)
}

func compare(a, b *triangle) int {
	if a.points[1][2] < b[1][2] {
		return -1
	}
	if a[1][2] > b[1][2] {
		return 1
